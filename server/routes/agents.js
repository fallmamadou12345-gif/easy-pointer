...
router.post('/', (req, res) => {
  ...
  if (zones?.length) {
    zones.forEach(z => db.prepare('INSERT OR IGNORE INTO agent_zones (agent_id,zone_id,est_defaut) VALUES (?,?,?)')
...
router.put('/:id', (req, res) => {
  ...
  db.prepare('UPDATE agents SET nom=?,prenom=?,poste=?,role=?,contrat=?,horaire=?,tel=?,email=?,date_embauche=?,actif=?,photo_base64=?,zone_defaut=?,updated_at=? WHERE id=?')
    .run(nom, prenom||'', poste||'', role||'Agent', contrat||'CDI', horaire||'08:00-16:00', tel||'', email||'', date_embauche||'', actif??1, photo_base64||null, zone_defaut||'', now, req.params.id);
  if (zones !== undefined) {
