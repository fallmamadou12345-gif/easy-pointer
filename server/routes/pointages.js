...
router.get('/today', (req, res) => {
  ...
  const pointages = db.prepare(`
    SELECT p.*,a.nom,a.prenom,a.poste,a.photo_base64,z.nom as zone_nom,z.couleur as zone_couleur
...
router.post('/scan', (req, res) => {
  ...
  if (lat && lon && agentZones.length > 0 && !zoneValidee) {
    ...
    return res.status(403).json({
      error: 'HORS_ZONE',
      message: `Vous êtes à ${Math.round(distanceMin)}m (max: ${agentZones[0]?.rayon}m)`,
  ...
  if (!existing) {
    db.prepare('INSERT INTO pointages (id,agent_id,date,arrivee,zone_id,lat,lon,distance,selfie,statut) VALUES (?,?,?,?,?,?,?,?,?,?)')
      .run(uuidv4(), agent_id, today, time, zoneValidee?.id||null, lat||null, lon||null, Math.round(distanceMin)||null, selfie?1:0, 'valide');
    action = 'arrivee';
